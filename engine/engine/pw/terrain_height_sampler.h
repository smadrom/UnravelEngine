...
struct terrain_heightfield
{
    ...
    [[nodiscard]] auto sample_terrain_normalized(float world_x, float world_z, float& out_height) const -> bool
    {
        ...
        const uint32_t col = stitch_block_grid == 0 ? 0 : x0 % stitch_block_grid;
        const uint32_t row = stitch_block_grid == 0 ? 0 : z0 % stitch_block_grid;
        const bool reversed = stitch_block_grid == 0 ||
                              (col == stitch_block_grid - 1 && row == 0) ||
                              (col == 0 && row == stitch_block_grid - 1);
        if(!reversed && tx > tz)
        {
            out_height = h00 + (h10 - h00) * tx + (h11 - h10) * tz;
        }
        else if(!reversed)
        {
            out_height = h00 + (h11 - h01) * tx + (h01 - h00) * tz;
        }
        else if(tx + tz <= 1.0f)
        {
